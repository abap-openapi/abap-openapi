CLASS zcl_client028 DEFINITION PUBLIC.
* Auto generated by https://github.com/abap-openapi/abap-openapi
* Title: Test 028
* Version: 1.0.0
  PUBLIC SECTION.
    INTERFACES zif_interface028.
...
CLASS zcl_client028 IMPLEMENTATION.
  METHOD constructor.
    mi_client = ii_client.
  ...
  METHOD zif_interface028~get_user.
    ...
    lv_uri = mv_uri_prefix && '/user/{id}'.
    REPLACE FIRST OCCURRENCE OF '{id}' IN lv_uri WITH id.
...
* todo, error handling
  ...
  METHOD zif_interface028~get_group_user.
    DATA lv_uri          TYPE string.
    DATA ls_header       LIKE LINE OF mt_extra_headers.
    DATA lv_dummy        TYPE string.
    DATA lv_content_type TYPE string.
    ...
    mi_client->request->set_version( if_http_request=>co_protocol_version_1_1 ).
    mi_client->request->set_compression( ).
    lv_uri = mv_uri_prefix && '/group/{groupId}/user/{userId}'.
    REPLACE FIRST OCCURRENCE OF '{groupId}' IN lv_uri WITH group_id.
    REPLACE FIRST OCCURRENCE OF '{userId}' IN lv_uri WITH user_id.
    cl_http_utility=>set_request_uri(
      request = mi_client->request
      uri     = lv_uri ).
    LOOP AT mt_extra_headers INTO ls_header.
