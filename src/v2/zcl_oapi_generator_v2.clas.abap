CLASS zcl_oapi_generator_v2 DEFINITION PUBLIC.
  ...
  PRIVATE SECTION.
    ...
    METHODS find_parameter_type
      IMPORTING is_parameter   TYPE zif_oapi_specification_v3=>ty_parameter
...
CLASS zcl_oapi_generator_v2 IMPLEMENTATION.
  ...
  METHOD build_clas_icf_serv.
    ...
    LOOP AT ms_specification-operations INTO ls_operation.
      ...
      IF ls_operation-request_body-schema_ref IS NOT INITIAL.
        rv_abap = rv_abap &&
          |          DATA { ls_operation-abap_name } TYPE { ms_input-intf }=>{ find_schema(
                                                                                   ls_operation-request_body-schema_ref )-abap_name }.\n| &&
      ...
      LOOP AT ls_operation-responses INTO ls_response.
        IF ls_response-code = 'default'.
          IF line_exists( ls_operation-responses[ code = '200' ] ).
            lv_code = '400'.
          ELSE.
            lv_code = '200'.
          ENDIF.
        ...
        IF lines( ls_response-content ) > 0.
          ...
          LOOP AT ls_response-content INTO ls_content.
            ...
            IF    ls_content-type  = 'application/json'
              ...
              lv_name = lo_response_name->generate_response_name( iv_content_type = ls_content-type
                                                                  iv_code         = ls_response-code ).
              rv_abap = rv_abap &&
                |            /ui2/cl_json=>deserialize(\n| &&
                |              EXPORTING\n| &&
  ...
  METHOD find_input_parameters.
    ...
    rv_abap = concat_lines_of( table = lt_list
                               sep   = |\n| ).
  ...
  METHOD find_returning_parameter.
    ...
    LOOP AT is_operation-responses ASSIGNING <ls_response>.
      ...
      ENDLOOP.
    ENDLOOP.
    rs_returning-type = |  TYPES: BEGIN OF { lv_typename },\n| &&
                        |           code          TYPE i,\n| &&
                        |           reason        TYPE string,\n| &&
                        |{ rs_returning-type }| &&
                        |         END OF { lv_typename }.\n|.
  ...
  METHOD split_description.
    ...
    DATA lt_descr2     TYPE STANDARD TABLE OF string WITH DEFAULT KEY.
    DATA lv_first_time TYPE abap_bool.

    FIELD-SYMBOLS <ls_desc1> TYPE string.
    FIELD-SYMBOLS <ls_desc2> TYPE string.
  ...
  METHOD split_string.
    ...
    DO lv_size TIMES.
      CLEAR lv_substring.
      lv_substring = substring( val = iv_input
                                off = lv_pos
                                len = iv_size ).
